...
/**
 * Servlet implementation class Proforma
 */
@WebServlet("/Proforma")
public class Proforma extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if (request.getSession().getAttribute("adminLog") != null)
		{
			...
			{
				//Getting ctive admin's details
				String admin_fname = (String) request.getSession().getAttribute("adminfname");
				String admin_sname = (String) request.getSession().getAttribute("adminoname");
				String admin_email = (String) request.getSession().getAttribute("adminemail");
				String admin_phone = (String) request.getSession().getAttribute("adminphone");
				String admin_company = (String) request.getSession().getAttribute("admincompany_name");
				...
				try
				{
					...
					while(rss.next())
					{
					    ...
					    out.print("<table width=25% frame=\"box\">");
					    out.print("<tr><td nowrap style=\"text-align: right\">COMPANY NAME &nbsp;</td><td>"+admin_company+"</td></tr>");
					    out.print("<tr><td style=\"text-align: right\">ATT &nbsp;</td><td nowrap>"+admin_fname+" "+admin_sname+"</td></tr>");
					    out.print("<tr><td style=\"text-align: right\"> Email &nbsp;</td><td>"+admin_email+"</td></tr>");
					    out.print("<tr><td style=\"text-align: right\">Telephone &nbsp;</td><td>"+admin_phone+"</td></tr>");
					    out.print("<tr><td style=\"text-align: right\">Fax &nbsp;</td><td></td></tr>");
					    out.print("</table>");
						...
						String proforma_details = "select distinct jobcard_details.jobcard_no,jobcard_details.company,jobcard_details.date_filled,jobcard_details.ticket,jobcard_details.report,jobcard_details.location,jobcard_used_items.pay,user_id,billable,reschedule from jobcard_details inner join jobcard_used_items on jobcard_details.jobcard_no = jobcard_used_items.jobcard_no inner join jobcard_charges on jobcard_details.jobcard_no = jobcard_charges.jobcard_no where jobcard_details.date_filled >= ? and jobcard_details.date_filled <= ? and jobcard_details.contractor_id=? and jobcard_details.status=? order by jobcard_details.jobcard_no";
